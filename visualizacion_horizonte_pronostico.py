...
def _mascara_pronostico(frame: pd.DataFrame, today: pd.Timestamp) -> pd.Series:
    ...
    for column in (
        "TipoDato", "TIPODATO", "TIPO", "Fuente", "FUENTE",
        "CalidadDato", "CALIDADDATO", "Calidad", "CALIDAD",
    ):
        if column in frame.columns:
            text = frame[column].astype("string").fillna("").str.lower()
            mask = mask | text.str.contains("pronost", regex=False)

    # El horizonte operativo nunca incluye fechas anteriores a hoy.
    mask = mask & (frame["Fecha"] >= today)
    if bool(mask.any()):
        return mask

    # Respaldo para motores que no conservan las columnas de procedencia.
    return frame["Fecha"] >= today


def mostrar_horizonte_pronostico(data: Any, site_name: str) -> None:
    """Muestra todo el horizonte de emergencia ya disponible, sin ampliarlo."""
    ...
    forecast = (
        forecast.groupby("Fecha", as_index=False, sort=True)["EMERREL"]
        .max()
    ...
    st.caption(
        f"{site_name}: {len(forecast)} días meteorológicos disponibles, "
    ...
    labels = [
        f"{value:.5f}" if abs(float(value)) < 0.001 else f"{value:.3f}"
        for value in forecast["EMERREL"]
    ]
    ...
    figure.update_layout(
        ...
        title={
            "text": f"Detalle diario del pronóstico de emergencia · {site_name}",
            "x": 0.0,
            "xanchor": "left",
        },
        xaxis={
