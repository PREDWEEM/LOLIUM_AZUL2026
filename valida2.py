...
# ---------------------------------------------------------
def optimizar_parametros_hidricos_3d(df_meteo, df_campo, modelo_ann, latitud_azul=-36.87, rango_ventanas=[7]):
    df = df_meteo.copy()
...
st.title("🌾 PREDWEEM by GUILLERMO R. CHANTRE — AZUL (BA) lat=-36.87 lon=-59.89")
    ...
    if st.button("Ejecutar Barrido 3D"):
        if df_meteo_raw is not None and df_campo_raw is not None and modelo_ann is not None:
            ...
            with st.spinner(f'Ejecutando iteraciones 3D en ventanas: {ventanas_a_probar}...'):
                ...
                tabla_optima = optimizar_parametros_hidricos_3d(df_meteo_opt, df_campo_opt, modelo_ann, latitud_azul=-36.87, rango_ventanas=ventanas_a_probar)
...
if df_meteo_raw is not None and modelo_ann is not None:
    ...
    # Bypass Ruptura Temprana (Valor específico para Azul: 0.75)
    ...
    st.sidebar.download_button("📥 Descargar Reporte Completo", output.getvalue(), "PREDWEEM_Integral_Azul_vK4_9_15_UX.xlsx")
