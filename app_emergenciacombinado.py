# -*- coding: utf-8 -*-
# ===============================================================
# 🌾 PREDWEEM INTEGRAL vK4.9.15 VISUAL V3 — LOLIUM AZUL 2026
...
st.caption("Azul · VISUAL V3 · ventana eficiente verde · exportación PNG de alta resolución")
...
# --- SIDEBAR ---
st.sidebar.image("https://raw.githubusercontent.com/PREDWEEM/LOLIUM_AZUL2026/main/logo.png", width="stretch")
...
if df_meteo_raw is not None and modelo_ann is not None:
    ...
    # 2. Choque Hídrico de Ruptura Temprana (45 mm por defecto; luego del día 45)
    ...
    st.plotly_chart(go.Figure(data=go.Heatmap(z=[df["EMERREL"].values], x=df["Fecha"], y=["Emergencia"], colorscale=colorscale_hard, zmin=0, zmax=1, showscale=False)).update_layout(height=120, margin=dict(t=30, b=0, l=10, r=10), title="Mapa de Riesgo Temporal (Tasa Diaria Azul)"), width="stretch")
    ...
    with tab1:
            ...
            st.plotly_chart(
                ...
                config={
                    ...
                    "toImageButtonOptions": {
                        "format": "png",
                        "filename": "PREDWEEM_Azul_dinamica_emergencia",
                        "height": 1000,
                        "width": 2000,
                        "scale": 2,
                    },
        ...
        if df_campo is not None and not df_sincronizado.empty:
            ...
            with col_curva:
                ...
                st.plotly_chart(fig_acum.update_layout(title="Dinámica de Llenado (Curvas Acumuladas Puras)", xaxis_title="Fechas", yaxis_title="Emergencia Acumulada (%)", height=430, hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)), width="stretch")
            ...
            with col_disp:
                ...
                with tab_flujos:
                    ...
                    st.plotly_chart(fig_1to1.update_layout(title="Ajuste 1:1 de Flujos por Evento", xaxis_title="Observado Relativo", yaxis_title="Simulado Relativo", height=380, showlegend=False, margin=dict(t=40, b=0, l=0, r=0)), width="stretch")
