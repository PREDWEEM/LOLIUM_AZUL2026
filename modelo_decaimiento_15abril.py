# -*- coding: utf-8 -*-
"""Decaimiento tardío de PREDWEEM Azul desde el 15 de abril.
...
No altera pesos ni entradas de la ANN.
"""
...
def factor_decaimiento_15abril(
    ...
    """Devuelve factor 1 antes del 15-abr y Weibull desde esa fecha."""
...
def _reemplazar_unico(source: str, old: str, new: str, etiqueta: str) -> str:
    ...
    if cantidad != 1:
        raise RuntimeError(
            f"Parche de decaimiento Azul no aplicado: '{etiqueta}' aparece "
...
def parchear_modelo_decaimiento_15abril(source: str) -> str:
    """Inserta techo 50 % + decaimiento desde 15-abr dentro del motor Azul."""
    ...
    # Decaimiento tardío Azul: hasta 14-abr EMERREL queda intacto.
    ...
    tau_d = max(
        float(tau_decaimiento) if tau_decaimiento is not None else 60.0,
        1e-9,
    )
    beta_d = max(
        float(beta_decaimiento) if beta_decaimiento is not None else 1.0,
        1e-9,
    )
    intensidad_d = float(np.clip(
        intensidad_decaimiento if intensidad_decaimiento is not None else 0.75,
        0.0,
        1.0,
    ))
    fraccion_max_d = 0.50
    ...
    for pos, (_, fila) in enumerate(df.iterrows()):
        if bool(mascara_decay.iloc[pos]):
            ...
            if max_pre > 0.0:
                techo_decay[pos] = (
                    fraccion_max_d * max_pre * factor_decay[pos]
                )
    ...
    if mascara_con_techo.any():
        valores_originales = (
            df.loc[mascara_con_techo, "EMERREL"]
            .clip(lower=0.0)
            .to_numpy()
        )
        techos_activos = df.loc[
            mascara_con_techo,
            "Techo_EMERREL_15Abr",
        ].to_numpy()
